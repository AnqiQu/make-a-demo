...
export type DaytonaOpenCodeDraftCompositeReviewerOptions = {
...
/** Implements the Compositing-stage review contract over a shared OpenCode session. */
export class DaytonaOpenCodeDraftCompositeReviewer {
  private readonly options: DaytonaOpenCodeDraftCompositeReviewerOptions;

  constructor(options: DaytonaOpenCodeDraftCompositeReviewerOptions) {
  ...
  async review(
    ...
    this.options.onStatus(
      `Draft Composite review attempt ${input.attempt} starting in session ${input.opencodeSessionID}.`,
    );

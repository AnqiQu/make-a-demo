...
function makeADemoToolsPluginContent(): string {
  return [
    ...
    "        args: {",
    '          manifest: preparationManifestSchema().describe("Draft Preparation Manifest to validate."),',
    "        },",
    "        async execute(args) {",
    "          await mkdir(dirname(validationRequestPath), { recursive: true })",
    '          await writeFile(validationRequestPath, JSON.stringify({ manifest: args.manifest }, null, 2), "utf8")',
    ...
    '          manifest: preparationManifestSchema().optional().describe("Required when status is succeeded. Must match the MakeADemo Preparation Manifest schema."),',
    ...
    "            await assertValidationPassed(args.manifest)",
    "          }",
    ...
    "}",
    "",
    "function preparationManifestSchema() {",
    ...
    "}",
    "",
    "async function assertValidationPassed(manifest) {",
    "  let validation",
    "  try {",
    ...
    "  if (!validatedManifest || validatedManifest.demoCommand !== manifest.demoCommand || validatedManifest.url !== manifest.url || validatedManifest.workspaceId !== manifest.workspaceId) {",
    '    throw new Error("Submitted manifest must match the latest passed validation manifest for demoCommand, url, and workspaceId.")',
    "  }",
    "}",
    "",
  ].join("\n");

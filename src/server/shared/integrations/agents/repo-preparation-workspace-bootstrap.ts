...
import { makeADemoArtifactDirectory } from "./repo-preparation-artifact-handoff";

const configDirectory = "/tmp/makeademo/opencode";
...
export async function bootstrapRepoPreparationWorkspace(input: {
  ...
  if (parentClone.exitCode !== 0) {
    ...
    return {
      failure: createRepoCloneFailure(parentClone, "parent OpenCode workspace"),
    };
  }
  ...
  if (config.exitCode !== 0) {
    throw new Error(
      `Failed to install MakeADemo OpenCode config: ${[config.stderr, config.stdout].filter(Boolean).join("\n")}`,
    );
  }
...
export function createRepoPreparationOpenCodeEnv(): Record<string, string> {
...
function createWriteConfigCommand(): string {
  const commands = [
    `mkdir -p ${shellQuote(makeADemoArtifactDirectory)} ${shellQuote(configDirectory)}`,
  ];
  for (const file of createMakeADemoOpenCodeConfigFiles()) {
    const destination = posix.join(configDirectory, file.path);
    commands.push(
